...
// check that interface is implemented at compile time
...
type Service interface {
	...
	ListAllSubnet(ctx context.Context, filter string) (*SubnetListIntentResponse, error)
...
// ListAllSubnet ...
func (op Operations) ListAllSubnet(ctx context.Context, filter string) (*SubnetListIntentResponse, error) {
...
// ListAllCluster ...
func (op Operations) ListAllCategoryValues(ctx context.Context, categoryKeyName, filter string) (*CategoryValueListResponse, error) {
	entities := make([]*CategoryValueStatus, 0)

	resp, err := op.ListCategoryValues(ctx, categoryKeyName, &CategoryListMetadata{
		Filter: &filter,
